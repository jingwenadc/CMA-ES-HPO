...
if __name__ == "__main__":
    ...
    # Generate the points
    learning_rates_list = np.random.uniform(CNN_LR[0], CNN_LR[1], num_points)
    n_dropout_list = np.random.uniform(CNN_DROPOUT[0], CNN_DROPOUT[1], num_points)
    n_layers_list = np.random.randint(CNN_LAYERS[0], CNN_LAYERS[1], num_points)
    n_neurons_list = np.random.randint(CNN_NEURONS[0], CNN_NEURONS[1], num_points)
    ...
    results, best_params, best_accuracy = random_search(param_combinations, objective_function)
    end_time = time.time()
    ...
    filename = f"results_rs_{job_timestamp}.pkl"
